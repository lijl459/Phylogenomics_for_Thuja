...
# using dendropy 3.12.3
# pip install --upgrade dendropy==3.12.3 --user
...
if len(sys.argv) != 3:
        print("Usage: <script> <tree1> <tree2>")
tree1 = sys.argv[1]
tree2 = sys.argv[2]
...
trees = dendropy.TreeList.get_from_path(tree1, "newick", taxon_set = taxon_set)
mrc = dendropy.Tree.get_from_path(tree2, "newick", taxon_set = taxon_set)
...
for k,v in master_dict.iteritems():
	...
	if k in mrc_dict.keys():
		...
		tax22 = taxon_set.split_taxa_list(k[1])
                print("\nparental internal branch")
                for i in tax11:
                        print(i)
                print("descendant internal branch")
                for i in tax22:
                        print(i)
		for i in tax11:
